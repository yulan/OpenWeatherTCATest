...
enum WeatherAPIClientError: Error, Equatable {
    ...
    case requestFailed              // 请求失败（如网络问题）
    case decodingFailed             // 数据解码失败
    case invalidResponse            // 非 200 状态码无效响应
    case apiError(message: String)  // API 返回的具体错误信息
    case unknown                    // 未知错误
...
struct MockWeatherRepository: WeatherRepositoryProtocol {
    func fetchWeather(lat: Double, lon: Double) async throws -> WeatherResponseDTO {
        ...
        return WeatherResponseDTO(
            coord: CoordDTO(lat: 48.893732, lon: 2.406402),
            weather: [
                WeatherConditionDTO(id: 800, main: "Clear", description: "clear sky", icon: "01d")
            ],
            main: MainDTO(temp: 18.5, feelsLike: 18.0, tempMin: 16.5, tempMax: 20.0, pressure: 1013, humidity: 60),
            wind: WindDTO(speed: 3.1, deg: 180, gust: nil),
            clouds: CloudsDTO(all: 0),
            sys: SysDTO(country: "FR", sunrise: 1650264000, sunset: 1650314400),
            name: "Paris",
            visibility: 10000,
